...
def labels_v4_default() -> Dict[str, Any]:
    """Return an empty schema v4 labels object (reviewed_at is null until first save)."""
    return {
        "schema_version": LABELS_SCHEMA_VERSION,
        "reviewed_at": None,
        "confirmed_by_type": {t: [] for t in DOOR_TYPES},
        "rejected_by_type": {t: [] for t in DOOR_TYPES},
...
def validate_labels_v4_or_raise(labels_data: Dict[str, Any], *, labels_path: Path) -> None:
    """Validate schema v4 labels.json. Raise with a clear migration message on failure."""
        ...
        raise ValueError(
            "\n".join(
                [
                    ...
                    "",
                    "This UI expects schema v4 labels. If this is an older schema,",
    ...
    missing = sorted(k for k in _LABELS_V4_REQUIRED_KEYS if k not in labels_data)
    if missing:
    ...
    if not isinstance(labels_data.get("rejected_by_type"), dict):
        raise ValueError(f"labels.json field 'rejected_by_type' must be an object: {labels_path}")
    ...
    for t in DOOR_TYPES:
        v = (labels_data.get("rejected_by_type") or {}).get(t)
        if v is None:
            continue
        if not isinstance(v, list):
            raise ValueError(f"labels.json rejected_by_type[{t!r}] must be a list: {labels_path}")


# Backwards-compatible aliases (older imports inside the repo may still reference v3 names).
def labels_v3_default() -> Dict[str, Any]:
    return labels_v4_default()


def validate_labels_v3_or_raise(labels_data: Dict[str, Any], *, labels_path: Path) -> None:
    return validate_labels_v4_or_raise(labels_data, labels_path=labels_path)
...
def flatten_rejected_ids(rejected_by_type: dict[str, set[str]]) -> set[str]:
    out: set[str] = set()
    if not isinstance(rejected_by_type, dict):
        return out
    for _, ids in rejected_by_type.items():
        try:
            out |= set(ids or set())
        except Exception:
            continue
...
def migrate_labels_v2_to_v4(labels_v2: Dict[str, Any]) -> Dict[str, Any]:
...
def migrate_labels_v3_to_v4(labels_v3: Dict[str, Any]) -> Dict[str, Any]:
    """Migrate schema v3 -> v4.

    v3 conflated all "delete" feedback into `deleted_ids`. We conservatively keep
    ...
    return {
        ...
        "confirmed_by_type": {t: list(cbt.get(t) or []) for t in DOOR_TYPES},
        "rejected_by_type": {t: [] for t in DOOR_TYPES},
        "deleted_ids": list(deleted_ids) if isinstance(deleted_ids, list) else [],
        "manual_additions": list(labels_v3.get("manual_additions", []) or []),
        "unmatched_manual_boxes": list(labels_v3.get("unmatched_manual_boxes", []) or []),
    }


# Backwards-compatible alias (repo previously used v2->v3 migration on load).
def migrate_labels_v2_to_v3(labels_v2: Dict[str, Any]) -> Dict[str, Any]:
    return migrate_labels_v2_to_v4(labels_v2)
...
def make_labels_payload_from_fstate(fstate: Dict[str, Any]) -> Dict[str, Any]:
    """Create schema v4 labels payload from an in-memory file state."""
    confirmed_by_type = coerce_confirmed_by_type(fstate.get("confirmed_by_type", {}))
    rejected_by_type = coerce_rejected_by_type(fstate.get("rejected_by_type", {}))
    return {
        "schema_version": LABELS_SCHEMA_VERSION,
