"""Regression test: post-reweight decisioning can rescue pool-only candidates.

This test runs `detect_doors()` directly with hand-crafted primitives.
"""
...
def test_post_reweight_rescues_pool_candidate() -> None:
    cx, cy = 100.0, 100.0
    r = 40.0

    # Leaf line intentionally too short for strict len_ratio but valid for pool.
    # NOTE: keep tip-to-arc distance safely under the pool threshold.
    ...
    with tempfile.TemporaryDirectory() as td:
        td_path = Path(td)
        model_path = td_path / "reweighter.json"
        ...
        cfg = {
            ...
            "reweighter_path": str(model_path),
            "output": {
        ...
        assert det.get("doors"), "expected post-reweight selection to keep a door"


def main() -> int:
    test_post_reweight_rescues_pool_candidate()
    print("✓ post-reweight selection test passed!")
    return 0
